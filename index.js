...
function roundObjectValues(obj) {
  ...
  return Object.fromEntries(
    Object.entries(obj).map(([KeyboardEvent, value]) => {
// Перевіряємо, чи значення є числом.
if (typeof value === "number") {
  return [KeyboardEvent, Math.round(value)];
}
...
console.log("Помилка: радіус і висота мають бути додатними числами.");
  // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
 return null;
}
...
function getFractionalPart(num) {
 ...
 const fractionalPart = num - integetPart;
...
function estimateInvestment(principal, interestRate, years) {
 ...
 const totalAmaunt = principal * Math.pow(1 + interestRate, years);
  // Округляємо ії до найближчого цілого
  const roundedAmount = Math.round(totalAmaunt);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  console.log("Rounded Max Price: ", roundedMaxPrice);
